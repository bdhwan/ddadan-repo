...
export function visibilityAwarePoll(
  ...
  if (
    ...
    (hiddenIntervalMs !== null &&
      (hiddenIntervalMs <= 0 || hiddenIntervalMs > 300_000))
  ) {
    throw new RangeError(
      "polling intervals must be positive and hidden polling cannot exceed 5 minutes",
    );
  }
  if (!documentRef)
    return interval(foregroundIntervalMs).pipe(
      startWith(0),
      map(() => undefined),
    );
  ...
  return visibility$.pipe(
    distinctUntilChanged(),
    switchMap((state) => {
      if (state === "hidden" && hiddenIntervalMs === null)
        return new Subject<void>();
      const delay =
        state === "hidden" ? hiddenIntervalMs! : foregroundIntervalMs;
