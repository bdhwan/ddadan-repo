...
@Component({
  ...
  template: `
    ...
    <div class="state-controls" role="group" aria-label="화면 상태 미리보기">
      <coupon-button variant="secondary" (click)="state.set('loading')">로딩</coupon-button>
    ...
    <coupon-card>
      @switch (state()) {
        @case ('loading') { <coupon-skeleton [lines]="5" /> }
        @case ('error') { <coupon-error-state requestId="phase1-demo" (retry)="state.set('loading')" /> }
        @default { <coupon-empty-state [title]="emptyTitle" [description]="emptyDescription" /> }
      }
    </coupon-card>
  `,
  styles: `
    .state-controls { display: flex; flex-wrap: wrap; gap: .5rem; margin-bottom: 1rem; }
  `,
})
export class CouponFeatureStateComponent {
  ...
  readonly description = String(this.route.snapshot.data['description'] ?? 'Phase 1 화면 shell입니다.');
  readonly emptyTitle = String(this.route.snapshot.data['emptyTitle'] ?? '아직 표시할 내용이 없어요');
  readonly emptyDescription = String(this.route.snapshot.data['emptyDescription'] ?? '데이터가 생기면 이곳에 보여드릴게요.');
}
