...
export function isWithinPeriod(value: Date | string, period: HalfOpenPeriod): boolean {
...
function timestamp(value: Date | string): number {
  ...
  if (Number.isNaN(result)) {
    throw new RangeError('period values must be valid dates or RFC3339 timestamps');
  }
  return result;
}
