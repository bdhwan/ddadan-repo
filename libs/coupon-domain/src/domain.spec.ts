...
describe("coupon domain formatters", () => {
  it("formats integer KRW, Seoul time, D-day and stamp boards", () => {
    ...
    expect(formatKoreaDateTime("2026-08-10T06:00:00Z")).toBe(
      "2026. 8. 10. 오후 3:00",
    );
    expect(
      formatExpiryDday("2026-08-11T14:59:59Z", "2026-08-10T06:00:00Z"),
    ).toBe("D-1");
...
describe("discount previews", () => {
  it("caps fixed discounts at the order amount", () => {
    expect(
      previewDiscount(8_000, { type: "FIXED", discount_amount: 10_000 })
        .discount_amount,
    ).toBe(8_000);
  });
