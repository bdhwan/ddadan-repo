...
export function reduceWalletState(
  ...
  switch (event.type) {
    ...
    case "SUCCESS":
      return {
        ...event.snapshot,
        status:
          event.snapshot.coupons.length || event.snapshot.stamps.length
            ? "ready"
            : "empty",
        message: null,
        read_only: false,
    ...
    case "OFFLINE":
      return event.cached
        ? {
            ...event.cached,
            status: "offline",
            message: "오프라인입니다. 마지막 동기화 내역만 볼 수 있습니다.",
            read_only: true,
          }
        : {
            ...EMPTY_WALLET_SNAPSHOT,
            status: "offline",
            message: "온라인 연결 후 지갑을 불러올 수 있습니다.",
            read_only: true,
          };
    case "FAILURE":
      if (event.cached) {
        return {
          ...
          message: event.online
            ? "최신 정보를 가져오지 못해 마지막 동기화 내역을 표시합니다."
            : "오프라인입니다. 마지막 동기화 내역만 볼 수 있습니다.",
          read_only: true,
        };
      }
      return {
        ...EMPTY_WALLET_SNAPSHOT,
        status: event.online ? "error" : "offline",
