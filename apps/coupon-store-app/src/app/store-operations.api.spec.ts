  ...
  it("reuses the explicit idempotency key while confirming an uncertain result", () => {
    ...
    const transportResponse = {
      data: {
        ...
        payable_amount: 9_000,
        status: "CONFIRMED",
        confirmed_at: "2026-08-10T06:01:00Z",
        voided_at: null,
        voidable_until: "2026-08-10T06:11:00Z",
      },
      request_id: "req-1",
      transaction_id: "tx-1",
    };
    const post = vi.fn().mockReturnValue(of(transportResponse));
    const injector = Injector.create({
      providers: [
        { provide: HttpClient, useValue: { post } },
        { provide: CatalogApi, useValue: { list: vi.fn() } },
      ],
    });
    ...
    for (const [url, body, options] of post.mock.calls) {
      ...
      expect(body.owner_session_id).toEqual(expect.any(String));
