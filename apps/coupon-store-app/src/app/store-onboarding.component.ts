...
@Component({
  ...
  template: `
    <header class="topbar">
      <a routerLink="/dashboard" aria-label="다단 상점 대시보드로 돌아가기"
        ><span aria-hidden="true">←</span> 대시보드</a
      >
      <coupon-badge [status]="reviewBadgeStatus()" [label]="reviewLabel()">{{
        reviewLabel()
      }}</coupon-badge>
    </header>
          ...
          @if (loading()) {
            ...
            <form [formGroup]="form" (ngSubmit)="saveStep(true)">
                ...
                @switch (step()) {
                  @case (0) {
                    <fieldset>
                      ...
                      <label
                        >상점명 <span aria-hidden="true">*</span
                        ><input
                  ...
                  @case (1) {
                    <fieldset>
                      ...
                      <label
                        >사업자등록번호 <span aria-hidden="true">*</span
                        ><input
                      ...
                      /></label>
                      <label
                        >대표자명 <span aria-hidden="true">*</span
                        ><input
                          formControlName="representative_name"
                          autocomplete="off"
                      /></label>
                      @if (store()) {
                        <div class="masked" role="status">
                          <strong>저장된 값</strong
                          ><span>{{
                            store()?.business_registration_number_masked ??
                              maskRegistration(
                                form.controls.business_registration_number.value
                              )
                          }}</span
                          ><span>{{
                            store()?.representative_name_masked ??
                              maskName(form.controls.representative_name.value)
                          }}</span>
                        </div>
                      }
                    </fieldset>
                  ...
                  @case (3) {
                    <fieldset>
                      ...
                      <label class="check"
                        ><input
                          type="checkbox"
                          formControlName="accepted_required_terms"
                        /><span
                          ><strong
                            >[필수] 상점 이용약관과 개인정보 처리방침</strong
                          ><small
                            >버전 phase1-2026-08 · 전문 링크는 배포 시
                            연결</small
                          ></span
                        ></label
                      >
                      <label class="check"
                        ><input
                          type="checkbox"
                          formControlName="accepted_marketing"
                        /><span
                          ><strong>[선택] 운영·마케팅 알림</strong
                          ><small
                            >동의하지 않아도 상점 서비스를 이용할 수
                            있습니다.</small
                          ></span
                        ></label
                      >
                  ...
                  @case (4) {
                    <fieldset>
                      ...
                      <dl>
                        <div>
                          <dt>상점명</dt>
                          <dd>{{ form.controls.name.value || "미입력" }}</dd>
                        </div>
                        <div>
                          <dt>주소</dt>
                          <dd>{{ form.controls.address.value || "미입력" }}</dd>
                        </div>
                        <div>
                          <dt>사업자번호</dt>
                          <dd>
                            {{
                              store()?.business_registration_number_masked ??
                                maskRegistration(
                                  form.controls.business_registration_number
                                    .value
                                )
                            }}
                          </dd>
                        ...
                        <div>
                          ...
                          <dd>
                            {{
                              store()?.representative_name_masked ??
                                maskName(
                                  form.controls.representative_name.value
                                )
                            }}
                          </dd>
                        </div>
                        <div>
                          <dt>영업시간</dt>
                          <dd>
                            {{ form.controls.opens_at.value }}–{{
                              form.controls.closes_at.value
                            }}
                          </dd>
                        </div>
                      </dl>
                      <ol class="timeline" aria-label="검수 상태">
                        <li class="done">
                          ...
                          <div>
                            <strong>초안 작성</strong>
                            <p>현재 정보를 안전하게 저장합니다.</p>
                          </div>
                        </li>
                        <li [class.done]="reviewSubmitted()">
                          <span aria-hidden="true">{{
                            reviewSubmitted() ? "✓" : "2"
                          }}</span>
                          <div>
                        ...
                        <li>
                          ...
                          <div>
                            <strong>승인·보완·거절</strong>
                            <p>운영 결정과 사유를 안내합니다.</p>
                          </div>
                        </li>
                      </ol>
                    </fieldset>
              ...
              <div class="actions">
                ...
                >
                @if (step() < 4) {
                  <coupon-button type="submit" [disabled]="saving()">{{
                    saving() ? "저장 중…" : "저장 후 다음"
                  }}</coupon-button>
                } @else {
                  <coupon-button
                    [disabled]="saving() || reviewSubmitted()"
                    (click)="submitReview()"
                    >{{
                      saving()
                        ? "제출 중…"
...
export class StoreOnboardingComponent implements OnInit {
  ...
  canLeave(): boolean {
    return (
      !this.isDirty() ||
      window.confirm("저장하지 않은 변경 내용이 있습니다. 화면을 나갈까요?")
    );
  }
      ...
      next: (response) => {
        ...
        if (advance) {
          this.step.update((value) =>
            Math.min(STEP_LABELS.length - 1, value + 1),
          );
        }
      },
      ...
      .subscribe({
        next: (response) => {
          ...
          this.statusMessage.set(
            "검수를 제출했습니다. 상태 변경을 이 화면에서 안내합니다.",
          );
        },
        error: (error: unknown) => this.captureError(error),
      });
  ...
  reviewBadgeStatus(): "success" | "warning" | "danger" | "neutral" {
    ...
    if (status === "CHANGES_REQUESTED" || status === "IN_REVIEW")
      return "warning";
  ...
  maskRegistration(value: string): string {
    ...
    return digits.length === 10
      ? `${digits.slice(0, 3)}-••-${digits.slice(-5)}`
      : "미저장";
  }
  ...
  private isCurrentStepValid(): boolean {
    const controlsByStep = [
      ...
      [
        this.form.controls.business_registration_number,
        this.form.controls.representative_name,
      ],
  ...
  private payload(): SaveOwnerStoreRequestDto {
    ...
    return {
      ...
      ...(value.accepted_required_terms
        ? { accepted_terms_version: "phase1-2026-08" }
        : {}),
  ...
  private applyStore(store: OwnerStoreDto): void {
    ...
    this.reviewSubmitted.set(
      store.review_status === "IN_REVIEW" || store.review_status === "APPROVED",
    );
  ...
  private restoreLocalDraft(): void {
    ...
    try {
      this.form.patchValue(
        JSON.parse(raw) as Partial<ReturnType<typeof this.form.getRawValue>>,
        { emitEvent: false },
      );
  ...
  private captureError(error: unknown): void {
    const safe =
      error instanceof Error ? error.message : "서버에 저장하지 못했습니다.";
...
export const onboardingLeaveGuard: CanDeactivateFn<StoreOnboardingComponent> = (
  component,
) => component.canLeave();
