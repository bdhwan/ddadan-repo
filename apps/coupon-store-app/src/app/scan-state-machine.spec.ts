...
describe("store scan state machine", () => {
  it("follows the complete required success path and waits for next customer", () => {
    ...
    machine.customerResolved();
    expect(machine.state).toBe("CUSTOMER_RESOLVED");
    machine.beginInput();
    machine.review();
    machine.submit();
    machine.succeed();
  ...
  it("moves submitting to failure and retries an uncertain result without resetting", () => {
    ...
    machine.submit();
    machine.fail();
    expect(machine.state).toBe("FAILURE");
    machine.retryUncertainSubmission();
